		...
		private static void ConfigureHttps(HttpsConnectionAdapterOptions options)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(certPath))
				{
					...
					Console.WriteLine($"Cert contents: \n {certString}");
					var certBytes = Convert.FromBase64String(certString);
					var httpsCert = new X509Certificate2(certBytes, (string)null, X509KeyStorageFlags.UserKeySet);// | X509KeyStorageFlags.EphemeralKeySet);
